...
- **设备管理**：支持设备请求/释放、占用冲突排队（FIFO）与释放后唤醒。
...
仓库内提供了示例程序：`t1.pc`（混合计算/访存/睡眠）、`t2.pc`（密集访存）、`t3.pc`（设备管理演示，适合同时创建两个同脚本进程观察设备申请/阻塞/唤醒）。例如 `t1.pc`：
```
# Simple test program
C
C
...
- 文件系统格式化、目录/文件创建与持久化读写
- `.pc` 文件驱动的文件操作
- 设备申请、阻塞排队与释放唤醒
- 密集访存触发缺页与页面置换

若需要单独演示设备管理，也可使用 `demo_device.pc`。
...
- 项目目标是“机制模拟”，不是完整内核；所有并发均为单线程逻辑并发。
- 文件系统当前支持普通文件、目录、直接块、持久化挂载；未实现目录递归删除、间接块、权限模型。
- `.pc` 中的 `FR/FW` 已接入真实文件系统，但不与进程虚拟内存内容做字节级联动。
- 设备管理当前采用简化设备表与 FIFO 等待队列，重点展示申请、阻塞、唤醒流程。
- Shell 中 `cat` 现支持跨多块顺序读取，但仍以教学演示为目标，不追求 Unix 命令兼容性。
